...
class Command(LMCPObject.LMCPObject):
    ...
    def pack(self):
        ...
        buffer.append(struct.pack(">q", self.CommandID))   # changed from l to q
    ...
    def unpack(self, buffer, _pos):
        ...
        self.CommandID = struct.unpack_from(">q", buffer, _pos)[0] # changed from l to q
        _pos += 8
        self.Time = struct.unpack_from(">q", buffer, _pos)[0] # changed from l to q
        _pos += 8
        ...
        _pos += 2
        if _arraylen > 0:
            self.EligibleEntities = struct.unpack_from(">" + `_arraylen` + "l", buffer, _pos )
            _pos += 8 * _arraylen
