...
from afrl.cmasi import ShieldInfo
from afrl.cmasi.SessionStatus import SessionStatus
from PyMASE import UAV, Location, get_args, MapViz
from Shield5 import Shield
import string
...
int_loc_map = { 1: (1.5467, -132.556),  2: (1.5467, -132.5623),  3: (1.5314, -132.5480),  4: (1.5467, -132.5479),\
                5: (1.5315, -132.5635),  6: (1.5315, -132.5577),  7: (1.5203, -132.5648),  8: (1.5196, -132.5562),\
                9: (1.5557, -132.5411), 10: (1.5484, -132.5328), 12: (1.531, -132.5227), 13: (1.5300, -132.5091),\
               11: (1.5255, -132.5375), 15: (1.5368, -132.5222), 14: (1.5370, -132.5100) }
...
uav_n = 2
UAVs = []
mapviz_green = MapViz(10,sock)
mapviz_red = MapViz(11,sock)
location = Location3D.Location3D()
location.set_Longitude(int_loc_map[6][1])
location.set_Latitude(int_loc_map[6][0])
...
try:
    while True:
        ...
        if (current_wp[0] <= int_loc_map[15][0] + 0.002 and current_wp[0] >= int_loc_map[15][0] - 0.002) and \
        (current_wp[1] <= int_loc_map[15][1] + 0.002 and current_wp[1] >= int_loc_map[15][1] - 0.002):
            if Fuel_monitor(UAVs[0]):
                UAVs[0].recharge()
        ...
        (current_wp[1] <= int_loc_map[corrected_next_wp][1] + 0.002 and current_wp[1] >= int_loc_map[corrected_next_wp][1] - 0.002):
            ...
            location_temp = int_loc_map[next_wp]
            next_wp = int_to_bin(next_wp - 1)
            print 'actual: ', (fuel_sensor, next_wp[0], next_wp[1], next_wp[2], next_wp[3])

            location = Location3D.Location3D()
            # location_temp = int_loc_map[bin_to_int(next_wp[3],next_wp[2],next_wp[1],next_wp[0])+1]
            location.set_Longitude(location_temp[1])
            location.set_Latitude(location_temp[0])

            mapviz_green.draw_circle(location, color = "green", radius = 200)
            info.set_Label('map, fuel')
            info.set_UAVID(1)
            

            corrected_next_wp = shield.move(fuel_sensor, next_wp[3], next_wp[2], next_wp[1], next_wp[0])
            ...
            print 'correction: ', corrected_next_wp
            location = Location3D.Location3D()
            location.set_Longitude(wp[1])
            location.set_Latitude(wp[0])
            mapviz_red.draw_circle(location, color = "red", radius = 200)
            move(UAVs[0], wp)
            packed_msg = LMCPFactory.packMessage(info, True)
            send = sock.send(packed_msg)
